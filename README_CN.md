...
| 语言 | 绑定机制 | 文件位置 |
|------|----------|----------|
| **C / C++** | 直接 `#include` | `src/pistadb.h` / `cpp/pistadb.hpp` |
| **Python** | `ctypes`（无 Cython） | `python/` |
| **Go** | CGO | `go/` |
| **Java** | JNI | `android/src/main/java/` |
| **Kotlin** | JNI + 扩展函数 | `android/src/main/kotlin/` |
| **Objective-C** | 直接 C 互操作 | `ios/Sources/PistaDBObjC/` |
| **Swift** | ObjC 桥接 | `ios/Sources/PistaDB/` |
| **C#** | P/Invoke | `csharp/` |
...
在 Android Studio 中将 `android/` 作为 Library 模块导入，或在 `settings.gradle` 中声明：

```groovy
include ':android'
project(':android').projectDir = new File('<PistaDB 路径>/android')
```

NDK 构建由 `android/CMakeLists.txt` 自动处理。请确认已安装 NDK `26.x`，且 `android/build.gradle` 中的 `ndkVersion` 与之匹配。

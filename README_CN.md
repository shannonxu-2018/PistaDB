...
所有七种索引类型现已统一使用 **VecStore** 分块存储层，替代了原先用于存储原始浮点向量与标签的单块连续内存分配方式。

| | 旧方案 | 新方案（VecStore） |
|---|---|---|
| 存储布局 | 单次 `realloc()` 增长至数十 GB | 每块 131,072 条记录，单次分配最大 64 MB |
| OOM 边界 | Windows 下约 9.4M 向量时触发（图节点碎片化堆内存） | **无** — 各块独立分配 |
| 单次最大分配 | 约 10 GB 连续内存 | 64 MB（dim=128 时一个 chunk） |
| 访问开销 | O(1) 指针运算 | O(1) 位移 + 掩码索引 |

分块对外完全透明——所有公开 API、`.pst` 文件格式及各语言绑定均无需变更。

### HNSW 验证至 1000 万向量

HNSW 在 Windows AMD64 环境下成功插入 **10,000,000 条向量**（`dim=128`，`M=8`，`ef_construction=50`），全程无 OOM。
...
HNSW 吞吐量随 N 对数下降，是图算法的固有特性；换来的是**任意规模下亚毫秒级近似 k-NN 查询**。
...
| **SEARCH** k=10 | 200 次查询 | — | p50=**316 ms** · p95=381 ms · QPS=3 |
| **UPDATE** | 100,000 | 6m 1s | **277 ops/s** |
| **DELETE** | 100,000 | 6m 6s | **273 ops/s** |

配置：`nlist=500`，`nprobe=50`，`dim=128`，Windows AMD64，69 GB RAM。

### 完整的生产级特性集

- **事务支持** — ACID 风格的原子多操作批次，失败时完整回滚
- **Android 集成** — 通过 JNI 提供 Java + Kotlin API（`android/`），支持全部 4 种 ABI
- **iOS / macOS 集成** — 通过 Swift Package Manager 提供 Objective-C + Swift API
...
测试环境：**9,000,000 条记录** · 向量维度 `dim=128` · L2 距离 · Windows AMD64 · Python 3.13 · 内存 69 GB

完整基准脚本：[`benchmarks/benchmark_10m.py`](benchmarks/benchmark_10m.py)

---

### IVF 索引 — 9M 条记录完整 CRUD

配置：`nlist=500`（聚类数），`nprobe=50`（每次查询探测 10% 的簇），训练样本 10 K 条。

| 操作 | 记录数 | 耗时 | 吞吐量 |
|------|--------|------|--------|
| **INSERT**（训练后写入） | 9,000,000 | 2m 26s | **61,494 vec/s** |
...
> **说明：** IVF 写入为 O(1)——每条向量仅需分配到最近的聚类中心，因此吞吐量在各规模下基本稳定。查询延迟在 `nprobe=50` 下反映了对 50 个聚类中约 18K 条向量的扫描（9M 规模下每次查询约 90 万次距离计算）；调小 `nprobe` 可降低延迟，召回率略有下降。UPDATE 和 DELETE 采用 O(N) 线性扫描定位目标记录，频繁修改的场景建议优先使用 HNSW（懒删除，无全表扫描）。

---

### HNSW 索引 — 千万级 INSERT 吞吐量
...
HNSW 写入代价随 N 对数增长（每次插入需遍历图结构，距离计算次数约为 O(M·log N)），因此吞吐量随数据量增大而缓慢下降。作为回报，HNSW 可提供**亚毫秒级近似 k-NN 查询**，与数据集规模无关，这是 IVF 在大 `nlist` 下无法达到的。

> **关于 10M 容量：** 旧版单块连续内存方案在 Windows 下约 9.4M 向量时触发 OOM（下一次容量翻倍需约 10 GB 的连续 `realloc()`）。该问题已通过 **VecStore 分块存储**彻底解决——无论数据集规模多大，单次最大分配仅为 64 MB。HNSW 已在 Windows AMD64 上验证可稳定插入 10,000,000 条向量（详见上方「最新特性」章节）。
...
- [ ] 完整浏览器 RAG 管道（WASM + IDBFS 持久化 + SharedArrayBuffer 多线程）
